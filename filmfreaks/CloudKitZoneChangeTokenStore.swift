...
enum CloudKitZoneChangeTokenStore {
    ...
    static func tokenKey(namespace: String, scope: Scope, zoneID: CKRecordZone.ID) -> String {
    ...
    static func token(namespace: String, scope: Scope, zoneID: CKRecordZone.ID, defaults: UserDefaults = .standard) -> CKServerChangeToken? {
    ...
    static func setToken(_ token: CKServerChangeToken?, namespace: String, scope: Scope, zoneID: CKRecordZone.ID, defaults: UserDefaults = .standard) {
        let key = tokenKey(namespace: namespace, scope: scope, zoneID: zoneID)
    ...
    static func clear(namespace: String, scope: Scope, zoneID: CKRecordZone.ID, defaults: UserDefaults = .standard) {
        let key = tokenKey(namespace: namespace, scope: scope, zoneID: zoneID)
        defaults.removeObject(forKey: key)
    }
}
